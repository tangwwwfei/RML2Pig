...
%default TriplesMap1_source '{"iterator":"$.results.bindings[*]","referenceFormulation":"ql:JSONPath","query":"PREFIX+rdfs%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F2000%2F01%2Frdf-schema%23%3E+PREFIX+foaf%3A+%3Chttp%3A%2F%2Fxmlns.com%2Ffoaf%2F0.1%2F%3E+PREFIX+rdf%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F1999%2F02%2F22-rdf-syntax-ns%23%3E+SELECT+%3FName+WHERE+%7B+%3Fx+foaf%3Aname+%3FName+.+%7D","sqlVersion":"","tableName":"","name":"","sparql":{"endpoint":"http://localhost:PORT/ds1/sparql","supportedLanguage":"sd:SPARQL11Query","resultFormat":"http://www.w3.org/ns/formats/SPARQL_Results_JSON"}}'

%default TriplesMap1_referenceFormulation 'ql:JSONPath'

%default TriplesMap1_iterator '$.results.bindings[*]'

%default TriplesMap1_template '<http://example.com/{Name.value}>'

TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.results.bindings[*].Name.value"]]') as (f1:tuple(chararray));
