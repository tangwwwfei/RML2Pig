...
TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["ID,FirstName"],["ID"],["FirstName"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray),f3:tuple(chararray));
...
TriplesMap1_subjectMap = foreach TriplesMap1_subjectMap generate 
	$0,
	'rdf:type',
	FLATTEN(R2PTOBAG('foaf:Person','ex:Student'));
