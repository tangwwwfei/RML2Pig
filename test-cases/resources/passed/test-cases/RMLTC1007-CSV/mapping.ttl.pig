...
%default map_map_data1_0_template '<http://example.com/1/{extra}>'

map_map_data1_0_data = load 'data.csv' using r2ps.udf.pig.R2PLOADSOURCE('${map_map_data1_0_source}','[["extra"],["id"],["name"]]') as (f1:tuple(chararray),f2:tuple(chararray),f3:tuple(chararray));
...
%default map_map_data2_0_template '<http://example.com/2/{extra}>'

map_map_data2_0_data = load 'data2.csv' using r2ps.udf.pig.R2PLOADSOURCE('${map_map_data2_0_source}','[["extra"],["id"],["name"]]') as (f1:tuple(chararray),f2:tuple(chararray),f3:tuple(chararray));
...
map_map_data1_0_predicateObjectMap = join child by ($1,$2), parent by ($1,$2);
