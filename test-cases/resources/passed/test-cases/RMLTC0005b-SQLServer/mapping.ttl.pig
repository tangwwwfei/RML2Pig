...
%default TriplesMap1_source '{"iterator":"","referenceFormulation":"","query":"","sqlVersion":"rr:SQL2008","tableName":"IOUs","name":"","database":{"jdbcDSN":"CONNECTIONDSN","jdbcDriver":"com.microsoft.sqlserver.jdbc.SQLServerDriver","username":"sa","password":"YourSTRONG!Passw0rd;"}}'

%default TriplesMap1_referenceFormulation ''

%default TriplesMap1_iterator ''

%default TriplesMap1_template '_:{fname}_{lname}'

TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["fname,lname"],["fname"],["lname"],["amount"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray),f3:tuple(chararray),f4:tuple(chararray));
...
TriplesMap1_subjectMap = foreach TriplesMap1_subjectMap generate 
	$0,
	'rdf:type',
	FLATTEN(R2PTOBAG('<http://example.com/base/IOUs>'));
