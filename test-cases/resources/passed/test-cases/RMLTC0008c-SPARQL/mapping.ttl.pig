...
TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.results.bindings[*].ID.value,$.results.bindings[*].Name.value"],["$.results.bindings[*].Name.value"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray));
