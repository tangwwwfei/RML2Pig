...
TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.results.bindings[*].ID.value"],["$.results.bindings[*].Name.value"],["$.results.bindings[*].Sport.value"]]') as (f1:tuple(chararray),f2:tuple(chararray),f3:tuple(chararray));
...
TriplesMap2_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["$.results.bindings[*].ID.value"],["$.results.bindings[*].Name.value"]]') as (f1:tuple(chararray),f2:tuple(chararray));
