...
%default TriplesMap1_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"","sqlVersion":"","tableName":"","name":"student_sport.csv"}'
...
TriplesMap1_data = load 'student_sport.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["Student"]]') as (f1:tuple(chararray));

TriplesMap1_table = foreach TriplesMap1_data generate 
	R2PFORMAT('${TriplesMap1_template}', $0),
	$0;

%default TriplesMap2_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"","sqlVersion":"","tableName":"","name":"student_sport.csv"}'
...
TriplesMap2_data = load 'student_sport.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["Sport"]]') as (f1:tuple(chararray));
