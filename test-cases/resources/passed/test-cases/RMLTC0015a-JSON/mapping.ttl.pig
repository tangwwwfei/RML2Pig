...
%default TriplesMap1_source '{"iterator":"$.countries[*]","referenceFormulation":"ql:JSONPath","query":"","sqlVersion":"","tableName":"","name":"country_en.json"}'

%default TriplesMap1_referenceFormulation 'ql:JSONPath'

%default TriplesMap1_iterator '$.countries[*]'

%default TriplesMap1_template '<http://example.com/{Code}>'

TriplesMap1_data = load 'country_en.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.countries[*].Code"],["$.countries[*].Name"]]') as (f1:tuple(chararray),f2:tuple(chararray));
...
%default TriplesMap2_source '{"iterator":"$.countries[*]","referenceFormulation":"ql:JSONPath","query":"","sqlVersion":"","tableName":"","name":"country_es.json"}'

%default TriplesMap2_referenceFormulation 'ql:JSONPath'

%default TriplesMap2_iterator '$.countries[*]'

%default TriplesMap2_template '<http://example.com/{Code}>'

TriplesMap2_data = load 'country_es.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["$.countries[*].Code"],["$.countries[*].Name"]]') as (f1:tuple(chararray),f2:tuple(chararray));
