...
TriplesMap1_data = load 'persons.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.persons[*].fname,$.persons[*].lname,$.persons[*].amount"],["$.persons[*].fname,$.persons[*].lname"],["$.persons[*].amount"]]') as (f1:tuple(chararray,chararray,chararray),f2:tuple(chararray,chararray),f3:tuple(chararray));
