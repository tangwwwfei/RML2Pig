...
TriplesMap1_data = load 'student.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["ID"],["FirstName"],["LastName"]]') as (f1:tuple(chararray),f2:tuple(chararray),f3:tuple(chararray));
...
TriplesMap2_data = load 'sport.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["ID"],["Description"]]') as (f1:tuple(chararray),f2:tuple(chararray));
...
%default LinkMap_1_2_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"","sqlVersion":"","tableName":"","name":"student_sport.csv"}'

%default LinkMap_1_2_referenceFormulation 'ql:CSV'

%default LinkMap_1_2_iterator ''

%default LinkMap_1_2_template '<http://example.com/student/{ID_Student}>'

LinkMap_1_2_data = load 'student_sport.csv' using r2ps.udf.pig.R2PLOADSOURCE('${LinkMap_1_2_source}','[["ID_Student"],["ID_Sport"]]') as (f1:tuple(chararray),f2:tuple(chararray));
