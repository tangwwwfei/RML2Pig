...
%default TriplesMap1_source '{"iterator":"$.persons[*]","referenceFormulation":"ql:JSONPath","query":"","sqlVersion":"","tableName":"","name":"ious.json"}'
...
TriplesMap1_data = load 'ious.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.persons[*].fname,$.persons[*].lname"],["$.persons[*].amount"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray));
