...
%default Country_template '<http://example.com/Student{ID.value}>'

Country_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${Country_source}','[["$.results.bindings[*].ID.value"],["$.results.bindings[*].FirstName.value,$.results.bindings[*].LastName.value"]]') as (f1:tuple(chararray),f2:tuple(chararray,chararray));
