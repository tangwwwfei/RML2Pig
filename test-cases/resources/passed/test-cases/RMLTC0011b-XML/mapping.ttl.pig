...
TriplesMap1_data = load 'student.xml' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["/Students/Student/ID"],["/Students/Student/FirstName"],["/Students/Student/LastName"]]') as (f1:tuple(chararray),f2:tuple(chararray),f3:tuple(chararray));
...
%default TriplesMap2_source '{"iterator":"/Sports/Sport","referenceFormulation":"ql:XPath","query":"","sqlVersion":"","tableName":"","name":"sport.xml"}'

%default TriplesMap2_referenceFormulation 'ql:XPath'

%default TriplesMap2_iterator '/Sports/Sport'

%default TriplesMap2_template '<http://example.com/sport/{ID}>'

TriplesMap2_data = load 'sport.xml' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["/Sports/Sport/ID"],["/Sports/Sport/Description"]]') as (f1:tuple(chararray),f2:tuple(chararray));
...
%default LinkMap_1_2_source '{"iterator":"/links/link","referenceFormulation":"ql:XPath","query":"","sqlVersion":"","tableName":"","name":"student_sport.xml"}'

%default LinkMap_1_2_referenceFormulation 'ql:XPath'

%default LinkMap_1_2_iterator '/links/link'

%default LinkMap_1_2_template '<http://example.com/student/{ID_Student}>'

LinkMap_1_2_data = load 'student_sport.xml' using r2ps.udf.pig.R2PLOADSOURCE('${LinkMap_1_2_source}','[["/links/link/ID_Student"],["/links/link/ID_Sport"]]') as (f1:tuple(chararray),f2:tuple(chararray));
