...
TriplesMap1_data = load 'persons.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.persons[*].ID,$.persons[*].Name,$.persons[*].DateOfBirth"],["$.persons[*].DateOfBirth"]]') as (f1:tuple(chararray,chararray,chararray),f2:tuple(chararray));
