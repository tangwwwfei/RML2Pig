...
%default TriplesMap1_source '{"iterator":"/students/student","referenceFormulation":"ql:XPath","query":"","sqlVersion":"","tableName":"","name":"student_sport.xml"}'
...
TriplesMap1_data = load 'student_sport.xml' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["/students/student/Student"]]') as (f1:tuple(chararray));

TriplesMap1_table = foreach TriplesMap1_data generate 
	R2PFORMAT('${TriplesMap1_template}', $0),
	$0;

%default TriplesMap2_source '{"iterator":"/students/student","referenceFormulation":"ql:XPath","query":"","sqlVersion":"","tableName":"","name":"student_sport.xml"}'

%default TriplesMap2_referenceFormulation 'ql:XPath'

%default TriplesMap2_iterator '/students/student'

%default TriplesMap2_template '<http://example.com/{Sport}>'

TriplesMap2_data = load 'student_sport.xml' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["/students/student/Sport"]]') as (f1:tuple(chararray));
