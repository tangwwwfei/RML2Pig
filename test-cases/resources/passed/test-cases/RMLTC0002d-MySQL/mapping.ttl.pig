...
%default TriplesMap1_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"SELECT+CONCAT%28%27Student%27%2C+ID%29+AS+StudentId%2C+ID%2C+Name+FROM+student","sqlVersion":"rr:SQL2008","tableName":"","name":"","database":{"jdbcDSN":"CONNECTIONDSN","jdbcDriver":"com.mysql.cj.jdbc.Driver","username":"root","password":""}}'

%default TriplesMap1_referenceFormulation 'ql:CSV'

%default TriplesMap1_iterator ''

%default TriplesMap1_template '_:{"StudentId"}'

TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["StudentId"],["Name"]]') as (f1:tuple(chararray),f2:tuple(chararray));
