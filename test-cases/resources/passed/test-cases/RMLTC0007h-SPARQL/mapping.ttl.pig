...
%default Country_source '{"iterator":"$.results.bindings[*]","referenceFormulation":"ql:JSONPath","query":"PREFIX+rdfs%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F2000%2F01%2Frdf-schema%23%3E%0D%0A++++++++++++PREFIX+foaf%3A+%3Chttp%3A%2F%2Fxmlns.com%2Ffoaf%2F0.1%2F%3E%0D%0A++++++++++++PREFIX+rdf%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F1999%2F02%2F22-rdf-syntax-ns%23%3E%0D%0A++++++++++++SELECT+%3FFirstName+%3FLastName+%3FID%0D%0A++++++++++++WHERE+%7B%0D%0A++++++++++++++++%3Fx++foaf%3AfirstName+%3FFirstName+%3B%0D%0A++++++++++++++++++++foaf%3AlastName++%3FLastName+%3B%0D%0A++++++++++++++++++++rdf%3AID+++++++++%3FID+.%0D%0A++++++++++++%7D","sqlVersion":"","tableName":"","name":"","sparql":{"endpoint":"http://localhost:PORT/ds1/sparql","supportedLanguage":"sd:SPARQL11Query","resultFormat":"http://www.w3.org/ns/formats/SPARQL_Results_JSON"}}'
...
Country_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${Country_source}','[["$.results.bindings[*].ID.value,$.results.bindings[*].FirstName.value"],["$.results.bindings[*].Name"],["$.results.bindings[*].Name.value"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray),f3:tuple(chararray));
...
Country_objectMap = foreach Country_objectMap generate 
	...
	'foaf:name',
	R2PFORMAT('"%s"',$3),
	FLATTEN(R2PTOBAG(R2PFORMAT('"%s"',$2)));
