...
%default TriplesMap1_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"SELECT+ID%2C+FirstName%2C+LastName+FROM+Employee+WHERE+ID+%3C+30","sqlVersion":"rr:SQL2008","tableName":"","name":"","database":{"jdbcDSN":"CONNECTIONDSN","jdbcDriver":"com.microsoft.sqlserver.jdbc.SQLServerDriver","username":"sa","password":"YourSTRONG!Passw0rd;"}}'

%default TriplesMap1_referenceFormulation 'ql:CSV'

%default TriplesMap1_iterator ''

%default TriplesMap1_template '<FirstName>'

TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["FirstName"]]') as (f1:tuple(chararray));
