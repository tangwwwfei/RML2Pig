...
%default TriplesMap2_source '{"iterator":"","referenceFormulation":"ql:CSV","query":"","sqlVersion":"","tableName":"","name":"student.csv"}'
...
TriplesMap2_data = load 'student.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["Sport"]]') as (f1:tuple(chararray));
...
TriplesMap1_data = load 'student.csv' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["ID,Name"],["ID"],["Name"],["Sport"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray),f3:tuple(chararray),f4:tuple(chararray));
