...
%default TriplesMap1_source '{"iterator":"","referenceFormulation":"","query":"SELECT+ID%2C+LastName%2C+CONCAT%28%60FirstName%60%2C+%27%27%2C+%60LastName%60%29+as+Name+FROM+student","sqlVersion":"rr:SQL2008","tableName":"student","name":"","database":{"jdbcDSN":"CONNECTIONDSN","jdbcDriver":"com.mysql.cj.jdbc.Driver","username":"root","password":""}}'

%default TriplesMap1_referenceFormulation ''

%default TriplesMap1_iterator ''

%default TriplesMap1_template '<http://example.com/{ID}/{LastName}>'

TriplesMap1_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["ID,LastName"],["Name"]]') as (f1:tuple(chararray,chararray),f2:tuple(chararray));
