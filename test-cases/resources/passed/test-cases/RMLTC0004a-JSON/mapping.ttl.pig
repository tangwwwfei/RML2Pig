...
%default TriplesMap1_source '{"iterator":"$.students[*]","referenceFormulation":"ql:JSONPath","query":"","sqlVersion":"","tableName":"","name":"student_sport.json"}'
...
TriplesMap1_data = load 'student_sport.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap1_source}','[["$.students[*].Student"]]') as (f1:tuple(chararray));

TriplesMap1_table = foreach TriplesMap1_data generate 
	R2PFORMAT('${TriplesMap1_template}', $0),
	$0;

%default TriplesMap2_source '{"iterator":"$.students[*]","referenceFormulation":"ql:JSONPath","query":"","sqlVersion":"","tableName":"","name":"student_sport.json"}'
...
TriplesMap2_data = load 'student_sport.json' using r2ps.udf.pig.R2PLOADSOURCE('${TriplesMap2_source}','[["$.students[*].Sport"]]') as (f1:tuple(chararray));
