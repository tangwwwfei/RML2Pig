...
%default Country_template '_:students{ID.value}'

Country_data = load 'UnusedPath' using r2ps.udf.pig.R2PLOADSOURCE('${Country_source}','[["$.results.bindings[*].ID.value"],["$.results.bindings[*].Name.value"]]') as (f1:tuple(chararray),f2:tuple(chararray));
